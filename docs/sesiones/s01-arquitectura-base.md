...
| 2. Persistencia y base de datos | 2 | Evidencia PostgreSQL DEV/PROD, Flyway y registros consultados con `psql`. | Evidencia tabla `categorias` y consultas basicas con `psql`. | Evidencia parcial de conexion a BD. | No evidencia uso de PostgreSQL ni tabla creada. | |
| 3. Ejecucion DEV y PROD local | 2 | Explica y evidencia DEV Maven, PROD Docker y escalado maximo a dos instancias. | Evidencia DEV con Maven y PROD local con Docker. | Muestra solo DEV o solo PROD local. | No diferencia DEV y PROD local. | |
